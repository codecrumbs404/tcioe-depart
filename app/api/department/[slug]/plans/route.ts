...
export async function GET(req: NextRequest, { params }: { params: { slug: string } }) {
  ...
  const publicUrl = `${API_BASE.replace(/\/$/, '')}${API_PUBLIC_PREFIX}/departments/${slug}/plans${qs}`
  ...
  const deptUrl = `${API_BASE.replace(/\/$/, '')}${API_PUBLIC_PREFIX}/departments/${slug}`
  const deptRes = await fetch(deptUrl, { headers: { accept: 'application/json' }, cache: 'no-store' })
  if (!deptRes.ok) {
    const text = await deptRes.text().catch(() => '')
    return new Response(text || 'Failed to load department', { status: deptRes.status })
  }
  const dept = await deptRes.json().catch(() => null as any)
  const deptName: string | undefined = dept?.name

  const cmsUrl = `${API_BASE.replace(/\/$/, '')}/api/v1/cms/department-mod/department-plans-policies${qs}`
  const cmsRes = await fetch(cmsUrl, { headers: { accept: 'application/json' }, cache: 'no-store' })
  if (!cmsRes.ok) {
    const text = await cmsRes.text().catch(() => '')
    return new Response(text || 'Failed to load plans', { status: cmsRes.status })
  }
  ...
  const filtered = deptName ? items.filter((it: any) => it?.department?.name === deptName) : items
